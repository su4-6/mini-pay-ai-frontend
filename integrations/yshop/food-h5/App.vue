...
import { APP_ID, IS_MINIPAY_FOOD } from '@/config'
import {
	initializeMiniPayFood,
	miniPayAuthorizationErrorCode,
	miniPayAuthorizationErrorMessage,
} from '@/utils/minipay-food-bridge'

let foodAuthorizationRunning = false

const authorizeMiniPayFood = async() => {
	if (foodAuthorizationRunning) return
	foodAuthorizationRunning = true
	try {
		await initializeMiniPayFood()
	} catch (error) {
		const code = miniPayAuthorizationErrorCode(error)
		console.warn(`[minipay-food] authorization failed code=${code}`)
		uni.showModal({
			title: '意向点餐登录失败',
			content: miniPayAuthorizationErrorMessage(error),
			confirmText: '重试',
			cancelText: '稍后再试',
			success: result => {
				if (result.confirm) authorizeMiniPayFood()
			},
		})
	} finally {
		foodAuthorizationRunning = false
	}
}

onLaunch(() => {
	console.log('App Launch')
	// 先恢复上次缓存的用户信息，避免页面闪现游客态/默认头像
	const cached = cookie.get('userinfo')
	if (cached && Object.keys(cached).length > 0) {
		main.SET_MEMBER(cached)
	}
	if (IS_MINIPAY_FOOD) {
		authorizeMiniPayFood()
	}
})
