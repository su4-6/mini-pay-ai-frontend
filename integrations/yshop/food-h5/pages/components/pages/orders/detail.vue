<template>
	...
	<view class="container order-detail-page">
		<view class="order-detail-page__body">
					...
					<list-cell :hover="false">
						<view class="w-100 d-flex align-items-center">
							...
							<view class="d-flex justify-content-end align-items-center w-40">
								...
								<view class="iconfont-yshop icon-location order-detail-page__shop-icon order-detail-page__shop-icon--last"></view>
							</view>
						</view>
					</list-cell>
					<!-- store info end -->
			...
			<view class="fixed-bottom flex justify-end bg-white p-2" v-if="IS_MINIPAY_FOOD && order.paid == 0">
				<view class="w-100"><uv-button type="primary" text="继续支付" @click="continuePayment(order)"></uv-button></view>
			</view>
			<view class="fixed-bottom flex justify-end bg-white p-2" v-else-if="order.paid > 0 && order.refundStatus == 0">
...
const continuePayment = async (currentOrder) => {
	try {
		const result = await requestNativeFoodPayment(currentOrder.orderId)
		if (result?.payload?.status === 'SUCCEEDED') await detail(currentOrder.orderId)
	} catch (_) {
