<template>
	...
	<view class="pay-page container position-relative">
		<view class="pay-page__content">
			...
			<!-- 支付方式 begin -->
			<view class="pay-page__payment" v-if="!IS_MINIPAY_FOOD">
			...
			<view class="pay-page__payment" v-else>
				<list-cell last :hover="false"><text>支付方式</text></list-cell>
				<list-cell last>
					<view class="pay-page__payment-row d-flex align-items-center justify-content-between w-100">
						<view class="iconfont iconbalance line-height-100 pay-page__payment-icon"></view>
						<view class="flex-fill">MiniPay 钱包余额</view>
						<view class="iconfont line-height-100 pay-page__checkbox pay-page__checkbox--checked iconradio-button-on"></view>
					</view>
				</list-cell>
			</view>
			<!-- 支付方式 end -->
...
import { createMiniPayFoodQuote, createMiniPayFoodOrder } from '@/api/minipay-food'
...
const miniPaySubmitting = ref(false)
...
const amount = computed(() =>{
	...
	// 加配送费
	if (orderType.value === 'takeout' && (IS_MINIPAY_FOOD || store.value.distance > 0)) {
		amount += parseFloat(store.value.deliveryPrice || 0);
	}
...
const pay = async() => {
	...
	if (IS_MINIPAY_FOOD) {
		if (miniPaySubmitting.value) {
			uni.hideLoading()
			return
		}
		if (amount.value <= 0) {
			uni.hideLoading()
			uni.showToast({ title: 'MiniPay 暂不支持零元订单', icon: 'none' })
			return
		}
		if (cart.value.some(item => !item.skuId)) {
			uni.hideLoading()
			uni.showModal({
				title: '请重新选择商品',
				content: '当前购物车来自旧版本，需清空后重新选择规格。',
				showCancel: false,
			})
			return
		}
		miniPaySubmitting.value = true
		try {
			const quote = await createMiniPayFoodQuote({
				shopId: store.value.id,
				addressId: orderType.value == 'takeout' ? address.value.id : null,
				fulfillmentType: orderType.value == 'takeout' ? 'TAKEOUT' : 'PICKUP',
				items: cart.value.map(item => ({ skuId: item.skuId, quantity: item.number })),
			})
			const idempotencyKey = `h5-${Date.now()}-${Math.random().toString(16).slice(2)}`
			const miniPayOrder = await createMiniPayFoodOrder({
				quoteId: quote.quoteId,
				remark: form.value.remark,
			}, idempotencyKey)
			main.DEL_COUPON()
			uni.hideLoading()
			requestNativeFoodPayment(miniPayOrder.externalOrderNo).catch(() => {})
		} catch (_) {
			miniPaySubmitting.value = false
			uni.hideLoading()
			uni.showToast({ title: '订单创建失败，请刷新后重试', icon: 'none' })
		}
		return
	}
