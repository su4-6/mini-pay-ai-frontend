...
import { requestMiniPayWalletBalance } from '@/utils/minipay-food-bridge'
import {
  userGetUserInfo,
  mineService
} from '@/api/user'
...
const services = ref([])
// MiniPay 构建下余额来自 MiniPay 钱包（原生桥接），否则显示 yshop 余额
const miniPayBalance = ref(null)
const displayBalance = computed(() => {
	if (IS_MINIPAY_FOOD && miniPayBalance.value !== null) {
		return miniPayBalance.value
	}
	return isLogin.value ? (member.nowMoney ?? 0) : 0
})
...
const getUserInfo = async() => {
	if (isLogin.value) {
		try {
			...
			// 网络/接口异常时保留上次成功的数据，不阻塞页面展示
			console.warn('getUserInfo failed:', error)
		}
	}
}

// 通过原生桥接获取 MiniPay 钱包余额（分转元），失败则回退显示 yshop 余额
const fetchMiniPayBalance = async () => {
	if (!IS_MINIPAY_FOOD || !isLogin.value) return
	try {
		const payload = await requestMiniPayWalletBalance()
		if (payload && payload.status === 'READY' && payload.availableAmountCent !== undefined) {
			miniPayBalance.value = (Number(payload.availableAmountCent) / 100).toFixed(2)
		}
	} catch (error) {
		console.warn('fetchMiniPayBalance failed:', error)
	}
}
