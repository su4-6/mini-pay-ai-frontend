...
fly.interceptors.response.use(
  ...
  error => {
    if (error.toString() == 'Error: Network Error') {
      // A transport failure does not prove that the member session is invalid.
      // In particular, the MiniPay handoff is deliberately unauthenticated and
      // must retain the current session so that it can be retried safely.
      return Promise.reject({ code: 'NETWORK_UNAVAILABLE', msg: '网络连接失败' })
    }
    if (error.status == 401) {
      handleLoginFailure()
...
function baseRequest(options) {
  ...
  // 结构请求需要的参数
  const { url, params, data, login, sensitive, ...option } = options
...
const request = ['post', 'put', 'patch'].reduce((request, method) => {
  ...
  request[method] = (url, data = {}, options = {}) => {
    if (!options.sensitive) logger.debug('[api]', method.toUpperCase(), url, data)
